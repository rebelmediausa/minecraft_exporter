...
var (
	binary = filepath.Join(os.Getenv("GOPATH"), "bin/minecraft_exporter")
)

const (
	address = "localhost:19940"
)

func TestFileDescriptorLeak(t *testing.T) {
	if _, err := os.Stat(binary); err != nil {
		t.Skipf("minecraft_exporter binary not available, try to run `make build` first: %s", err)
	}
...
func TestHandlingOfDuplicatedMetrics(t *testing.T) {
	...
	dir, err := os.MkdirTemp("", "minecraft-exporter")
	if err != nil {
		t.Fatal(err)
	}
