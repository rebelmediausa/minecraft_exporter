...
var (
	scrapeDurationDesc = prometheus.NewDesc(
		...
		"minecraft_exporter: Duration of a collector scrape.",
		[]string{"collector"},
		nil,
	)
	scrapeSuccessDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "scrape", "collector_success"),
		"minecraft_exporter: Whether a collector succeeded.",
		[]string{"collector"},
		nil,
	)
)
...
type MinecraftCollector struct {
...
func NewMinecraftCollector(logger *slog.Logger, filters ...string) (*MinecraftCollector, error) {
	...
	return &MinecraftCollector{Collectors: collectors, logger: logger}, nil
}

func (n MinecraftCollector) Describe(ch chan<- *prometheus.Desc) {
	ch <- scrapeDurationDesc
	ch <- scrapeSuccessDesc
}

func (n MinecraftCollector) Collect(ch chan<- prometheus.Metric) {
	wg := sync.WaitGroup{}
